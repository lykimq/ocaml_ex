...
let _group_by_source_file src_line counts m =
...
let write_profiling_result output_name (agg_result : aggregate_result) =
  ...
  let total_samples =
    Hashtbl.fold (fun _a b c -> c + b.self_time) agg_result 0
  in
  let key_values = flatten agg_result in
  (*let _only_present_filenames =
    List.filter
      (function
        | {filename= None}, _ ->
            false
  ...
  let sum_counts_by_line =
    fold_groups
      (fun _ l ->
        group_by_fold (fun (k, _v) -> k.line) l (fun _ cs -> sum_counts cs) )
      grouped_by_file_function
  in
  let hottest_file_functions =
    take
      (List.sort
         (fun (_k0, v0) (_k1, v1) -> v1.self_time - v0.self_time)
