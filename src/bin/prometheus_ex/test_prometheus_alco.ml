...
let test_metrics () =
  ...
  let requests = 
    ...
    Counter.v_labels ~label_names ~registry ~help:"Requests"
    ~namespace:"dkci"
    ~subsystem:"tests"
    "requests"
  in 
  ...
  "#HELP dkci_tests_requests Request\n\
   #TYPE dkci_test_requests counter\n\
   dkci_test_requests{method=\"GET\", path=\"\\\"\\\\-\\n\"} 5.000000\n\
